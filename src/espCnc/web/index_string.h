...
static constexpr const char * js_string = R"rawstring((() => {
  ...
  function coord_to_angle(xy) {
    ...
    if (theta_paper > 0.8 || theta_paper < -0.8) throw new Error("angle extra");
  ...
  var LineCommand = class {
    ...
    encode() {
      ...
      let substeps = Math.ceil(dist * 100);
      let cmds = [];
  ...
  var UI = class {
    ...
    upload_code() {
      ...
      const binary = new Int32Array(commands.flatMap((x) => x.encode()));
      this.ws.send(binary);
    }
  };
  ...
  canvas.addEventListener("mousedown", (ev) => {
    const xy = [ev.clientX - canvas.offsetLeft, ev.clientY - canvas.clientTop];
    if (!mouse) {
      mouse = xy;
    } else {
      let next_mouse = xy;
      ...
      console.log(mouse[0], mouse[1], next_mouse[0], next_mouse[1]);
      ctx.stroke();
